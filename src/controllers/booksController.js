...
async function filterBooks(req, res) {
  try {
    const filterCategories = req.query.categories === '' ? null : req.query.categories.split('_')
    const filterPrice = req.query.price === '' ? null : req.query.price

    let filteredBooks = []

    const sortBy = filterPrice === 'cheap' ? 'asc' : 'desc'
    const regexForCategories = new RegExp(`^${filterCategories.join('|')}$`, 'i')

    if(filterCategories && filterPrice){
      filteredBooks = await BooksDB.find({category: regexForCategories}).sort({price: sortBy})
    } else if(filterCategories) {
      filteredBooks = await BooksDB.find({category: regexForCategories})
    } else if(filterPrice) {
